...
from app.repositories.coachee_repo import CoacheeRepository
router = APIRouter()
...
def register_user(user: UserCreate, db: Session = Depends(get_db)):
    ...
    coachee_repo = CoacheeRepository(db)
    ...
    # 🔥 CREATE COACH PROFILE ONLY IF ROLE = coach
    if db_user.role == "coach":
        coach_repo.create_coach_profile(user_id=db_user.id)
    if db_user.role == "coachee":
        coachee_repo.create_coachee_profile(user_id=db_user.id)
