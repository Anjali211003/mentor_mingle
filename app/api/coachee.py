...
def update_coachee_profile(
    ...
    if current_user.role != "coachee":
        raise HTTPException(
            status_code=403,
            detail="Only coachees can update their profile"
        )

    # Check if the coachee profile exists
    existing_profile = (
        db.query(CoacheeProfile)
        .filter(CoacheeProfile.user_id == current_user.id)
        .first()
    )
    ...
    if not existing_profile:
        raise HTTPException(
            status_code=404,
            detail="Coachee profile not found"
        )
    ...
    if profile.expertise is not None:
        existing_profile.expertise = profile.expertise
    ...
    if profile.image_url is not None:
        existing_profile.image_url = str(profile.image_url)  # 🔥 FIX
